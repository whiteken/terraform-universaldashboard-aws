data "aws_ami" "LatestMicrosoftServerImage" {
  most_recent      = true
  name_regex       = "WindowsServer2012R2-hvm"
  owners           = ["amazon"]

  filter {
    name   = "name"
    values = ["aws-*x86_64-WindowsServer2012R2-hvm-*"]
  }
...
resource "aws_instance" "myec2" {
  ...
  ami = data.aws_ami.LatestMicrosoftServerImage.id
...
<powershell>
  ...
  Start-Process -FilePath "$env:temp\ConfigureRemoting.ps1" -Wait
